...
<header class="w-full fixed top-0 z-50 transition-all duration-300 bg-white shadow-md" id="mainHeader">
    ...
    <div class="bg-green-400 text-white text-sm py-2 px-6 hidden md:flex justify-between items-center">
        <div class="flex items-center gap-6">
            <span><i class="fa-solid fa-phone"></i> {{ $company->phone ?? '+123 456 7890' }}</span>
            <span><i class="fa-solid fa-envelope"></i> {{ $company->email ?? 'info@realestate.com' }}</span>
        </div>

        <div class="flex items-center gap-4">
            @if($company?->facebook)
                <a href="{{ $company->facebook }}" target="_blank" class="hover:text-yellow-400 transition"><i class="fa-brands fa-facebook"></i></a>
            @endif
            @if($company?->instagram)
                <a href="{{ $company->instagram }}" target="_blank" class="hover:text-yellow-400 transition"><i class="fa-brands fa-instagram"></i></a>
            @endif
            @if($company?->youtube)
                <a href="{{ $company->youtube }}" target="_blank" class="hover:text-yellow-400 transition"><i class="fa-brands fa-youtube"></i></a>
            @endif

        @if($company?->tiktok)
                <a href="{{ $company->tiktok }}" target="_blank" class="hover:text-yellow-400 transition"><i class="fa-brands fa-tiktok"></i></a>
            @endif
        </div>
    </div>
