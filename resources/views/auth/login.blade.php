    ...
    <div class="min-h-screen bg-linear-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center p-4 sm:p-6 lg:p-8">
        <div class="w-full max-w-md">
            ...
            <div class="bg-white/85 dark:bg-gray-800/85 backdrop-blur-xl shadow-2xl rounded-2xl border border-white/30 dark:border-gray-700/40 overflow-hidden transition-all duration-300 hover:shadow-3xl">
                ...
                <!-- Session Status -->
                <x-auth-session-status class="mt-4 text-center text-green-600 dark:text-green-400" :status="session('status')" />

                <!-- Form -->
                <form method="POST" action="{{ route('login') }}" class="px-8 pb-10 pt-6 space-y-6">
                    @csrf
                    ...
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <label for="remember_me" class="inline-flex items-center">
                            <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-gray-700" name="remember">
                            <span class="ms-2 text-sm text-gray-600 dark:text-gray-300">{{ __('Remember me') }}</span>
                        </label>

                        @if (Route::has('password.request'))
                            <a class="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 hover:underline transition duration-150" href="{{ route('password.request') }}">
                    ...
                    <!-- Login Button -->
                    <div class="pt-2">
                        <x-primary-button class="w-full justify-center bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg">
                            {{ __('Sign In') }}
                        </x-primary-button>
                    </div>
                    ...
                    <div class="mt-6 text-center text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <a href="{{ route('register') }}" class="font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 hover:underline transition duration-150">
                            {{ __('Sign up now') }}
                        </a>
                    </div>
                </form>
