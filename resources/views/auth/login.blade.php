<x-guest-layout :title="__('Welcome back')" subtitle="Sign in to continue exploring properties">
    <x-auth-session-status class="text-center text-sm text-emerald-600 dark:text-emerald-400" :status="session('status')" />
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div>
            ...
            />
            <x-input-error :messages="$errors->get('email')" class="text-destructive mt-2 text-sm" />
        </div>
        ...
        <div>
            ...
            />
            <x-input-error :messages="$errors->get('password')" class="text-destructive mt-2 text-sm" />
        </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <label for="remember_me" class="inline-flex items-center gap-2">
                <input
                    id="remember_me"
                    type="checkbox"
                    class="border-input text-primary focus:ring-ring size-4 rounded border shadow-xs focus:ring-2 focus:ring-offset-0"
                    name="remember"
                />
                <span class="text-muted-foreground text-sm">{{ __('Remember me') }}</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-primary text-sm font-medium hover:underline" href="{{ route('password.request') }}">
        ...
        <p class="text-muted-foreground text-center text-sm">
            ...
            <a href="{{ route('register') }}" class="text-primary font-medium hover:underline">{{ __('Sign up') }}</a>
        </p>
    </form>
</x-guest-layout>
